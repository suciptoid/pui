%{
  title: "Icons",
  description: "Provider-agnostic semantic icons with Heroicons, Lucide, Tabler, and custom markup integration.",
...
The exact provider names and Tailwind/SVG setup belong to the host
application; PUI only relies on the semantic token contract.

## Application-owned icons

Use slots when the icon represents application navigation, business meaning,
or an icon outside PUI's small semantic vocabulary:
...
The slot can contain a Phoenix Heroicon, a Lucide component, a Tabler
component, or custom SVG markup. PUI owns the surrounding layout and
accessibility relationship, while the host owns the icon library.

The published [Icons guide](https://hexdocs.pm/pui/guides/icons.html) contains
the same package documentation for consumers who are not using the demo
application.
